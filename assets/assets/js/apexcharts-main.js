async function fetchWeightList() {
    try {
        const response = await fetch('/getAllBodyWeight'); // Replace with your API URL
        
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }

        const result = await response.json(); // Parse the JSON response (array of objects)

        // Initialize an empty list (array) to store the weights
        let weightList = [];

        // Use a for loop to iterate over each object and extract the 'Weight' field
        for (let i = 0; i < result.length; i++) {
            weightList.push(parseFloat(result[i].Weight)); // Convert weight to a float and add to list
        }

        console.log('Extracted weight list:', weightList);
        
        // Call the function to render the chart with the fetched weight data
        renderChart(weightList);
    } catch (error) {
        console.error('Error fetching weight list:', error);
    }
}

// Function to render the ApexCharts chart
function renderChart(weightData) {
    // Generate an index array for the X-axis (1, 2, 3, ...)
    const categories = weightData.map((_, index) => index + 1);
    ...
    const options = {
        ...
        markers: {
            size: 4, // Makes data points visible
            colors: ['#1C1C1C'],
        },
        title: {
...
// Call the function to fetch and process the weight list
fetchWeightList();
